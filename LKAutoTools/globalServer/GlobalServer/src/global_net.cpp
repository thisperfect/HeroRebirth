...
int global_recvtoqueue( short server_code, short cmd, int value1, int value2, char *pBuf, int readsize )
{
	...
	queue_tail = g_global_recv_queue.m_nQueueTail + 1;
	if( queue_tail >= MAX_GLOBALQUEUENUM )
	{
		queue_tail = 0;
	}
	if( g_global_recv_queue.m_nQueueHead == queue_tail )
	{
		// 队列已经满了, 断开这个客户,已经是非常的危险时刻,放弃所有的数据包
		g_global_recv_queue.m_nQueueHead = g_global_recv_queue.m_nQueueTail;
		mmux_unlock( g_global_recv_mmux );
		write_netlog( "warning:close client(global read queue full)" );
		mcond_broadcast( g_pthr_work_global );
		return -1;
	}

	memcpy( g_global_recv_queue.m_queue[g_global_recv_queue.m_nQueueTail].buf, pBuf, readsize );

	g_global_recv_queue.m_queue[g_global_recv_queue.m_nQueueTail].buf_size = readsize;
	g_global_recv_queue.m_queue[g_global_recv_queue.m_nQueueTail].server_code = server_code;
	g_global_recv_queue.m_queue[g_global_recv_queue.m_nQueueTail].cmd = cmd;
	g_global_recv_queue.m_queue[g_global_recv_queue.m_nQueueTail].value[0] = value1;
	g_global_recv_queue.m_queue[g_global_recv_queue.m_nQueueTail].value[1] = value2;

	g_global_recv_queue.m_nQueueTail = queue_tail;
