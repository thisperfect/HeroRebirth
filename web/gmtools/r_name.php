...
echo"<th class='DataHeader'>头像</th>";
...
$sql="select id, serverid, cityid, headid, actorid, name, vip, sign, count(*) from report_name group by sign limit 50;";
$query = $db->query($sql);
while($row = $db->fetch_row($query))
{
	...
	$headpath = "http://kor.res.gohogame.com/head/verify/$serverid/$headfdate/$headfile";
	...
	echo"<td class='Data DataRightAlign'><img src='".$headpath."' height='64' width='64' /></td>";		
...
function opselect(obj)
{
	...
	case '19':
		...
		document.getElementById("pv2").innerHTML = "<select id='proc_value2' name='proc_value2'><option value='6278'>[6278]聊天警告</option><option value='6277'>[6277]头像警告</option><option value='6280'>[6280]名称警告</option></select>";
	...
	case '38':
		...
		break;
	case '64':
		document.getElementById("proc_value1").innerHTML = "<font color='red'>取消头像</font>";
		document.getElementById("pv1").innerHTML = "<input type='text' name='proc_value1' size='30' value='-1' readonly>";
		break;
	case '56':
