...
$g_clubtechkind = array(0=>'无');
$g_clubdragon = array(0=>'无');

function getdbdatalist( $db )
{
	Global $g_clubtechkind;
	Global $g_clubdragon;
	$result = 0;

	$sql="select kind, name from club_technology group by kind order by kind";
	$query = $db->query($sql);
	while($row = $db->fetch_row($query))
	{
		$g_clubtechkind[$row[0]] = $row[1];
	}
	
	$sql="select kind, dragon_skill.desc from dragon_skill group by kind order by kind";
	$query = $db->query($sql);
	while($row = $db->fetch_row($query))
	{
		$g_clubdragon[$row[0]] = $row[1];
	}
	return $result;
}
...
echo"<th class='DataHeader'>联盟ID</th>";
echo"<th class='DataHeader'>联盟名字</th>";
echo"<th class='DataHeader'>联盟简称</th>";
echo"<th class='DataHeader'>联盟语种</th>";
echo"<th class='DataHeader'>领主名称</th>";
echo"<th class='DataHeader'>创建时间</th>";
echo"<th class='DataHeader'>战力</th>";
echo"<th class='DataHeader'>最高人数</th>";
echo"<th class='DataHeader'>任务完成数</th>";
echo"<th class='DataHeader'>详细信息</th>";
echo"<th class='DataHeader'>行为日志</th>";
$recno = 2;
for ( $i = 0; $i < $num; $i++ )
{
	if(isset($array[$i]))
	{
		...
		if($row = $db->fetch_row($query))
		{
			...
			if( $hasactorid == 1 )
			{
				$sql="select clubid, name, prefix, leadername, language, from_unixtime(createtime), battlepower, member_maxcount, questcomplatenum from club where clubid = '$actorid' order by createtime asc limit 500;";
			}
			else
			{
				$sql="select clubid, name, prefix, leadername, language, from_unixtime(createtime), battlepower, member_maxcount, questcomplatenum from club where name like '%$systalk_info%' order by battlepower desc limit 500;";
			}
			$query = $db->query($sql);
			while($row = $db->fetch_row($query))
			{
				...
				$createtime=$row[5];
				$battlepower=$row[6];
				$member_maxcount=$row[7];
				$questcomplatenum=$row[8];
			
				echo"<tr class='Record_2'>";
				echo"<td class='Data DataRightAlign'><input name='favor[]' type='checkbox' value='$sysip:$sysport|$clubid' ></td>";
				echo"<td class='Data DataRightAlign'>$clubid</td>";
				echo"<td class='Data DataRightAlign'>$name</td>";
				echo"<td class='Data DataRightAlign'>$prefix</td>";
				echo"<td class='Data DataRightAlign'>$g_language[$language]</td>";
				echo"<td class='Data DataRightAlign'>$leadername</td>";
				echo"<td class='Data DataRightAlign'>$createtime</td>";
				echo"<td class='Data DataRightAlign'>$battlepower</td>";
				echo"<td class='Data DataRightAlign'>$member_maxcount</td>";
				echo"<td class='Data DataRightAlign'>$questcomplatenum</td>";
